...
def optical_flow_horn_schunck_3d(im1: np.ndarray, im2: np.ndarray, *, alpha: float = 0.001, niter: int = 8) -> (
    ...
    u_initial = np.zeros([im1.shape[0], im1.shape[1]], dtype=np.float32)
    v_initial = np.zeros([im1.shape[0], im1.shape[1]], dtype=np.float32)
    z_initial = np.zeros([im1.shape[0], im1.shape[1]], dtype=np.float32)
    ...
    return u.tolist(), v.tolist(), z.tolist()
